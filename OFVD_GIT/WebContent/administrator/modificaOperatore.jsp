	...
	function load() {
		...
		$.ajax({
			...
			success : function(jsonStr) {
				...
				var cognome = jsonStr['cognome'];
				var email = jsonStr['email'];
				var matricola = jsonStr['matricola'];
				var hospital = jsonStr['hospital'];
				var cav = jsonStr['cav'];
				var admin = jsonStr['admin'];
				var telefono = jsonStr['telefono'];

				$('#nome_operatore').val(nome);
				$('#cognome_operatore').val(cognome);
				$('#telefono_operatore').val(telefono);
				$('#email_operatore').val(email);
				$('#matricola_operatore').val(matricola);
				$('#codice_ospedale').val(hospital);
				$('#codice_cav').val(cav);
				$('#codice_admin').val(admin);
          ...
          <div class="col-sm-8">
            <div class="breadcrumbs-area clearfix">
              ...
              <ul class="breadcrumbs pull-left">
                ...
                <li><span>Modifica Operatore</span></li>
              </ul>
            </div>
          </div>
              ...
              <div class="card-body">
                ...
                <form method="post">
                  <div id="alertUpdate" class="alert alert-primary" role="alert" hidden="true">
                    ...
                    <hr>
                    <div align="center">
                      <input class="btn btn-rounded btn-primary mb-3" type="button" value="Conferma" onclick="updateUser();"> <input class="btn btn-rounded btn-primary mb-3" type="button" value="Annulla" onclick="nascondi();">
                    </div>
                  </div>
                  <div class="form-group">
                    ...
                    <label>Email&nbsp;<span id="email_operatore_error_msg" style="color: red;"></span></label> <input readonly id="email_operatore" type="text" class="form-control" name="email"> 
                    ...
                    <label class="form-label">Identificativo Servizio Antiviolenza&nbsp;<span id="cav_operatore_error_msg" style="color: red;"></span></label> <select class="custom-select" id="codice_cav">
                      ...
                      %>
                    </select> <label>Codice Admin</label> <input id="codice_admin" type="text" class="form-control" name="code_admin" readonly> <label>Password</label> <input id="password" class="form-control" type="password" name="password" readonly>
                  </div>
                  <button type="button" class="btn btn-success btn-lg btn-block" onclick="alertUpdate();">Modifica</button>
                  <button type="button" onclick="indietro();" class="btn btn-secondary btn-lg btn-block">Annulla</button>
                </form>
              </div>
            </div>
          </div>
